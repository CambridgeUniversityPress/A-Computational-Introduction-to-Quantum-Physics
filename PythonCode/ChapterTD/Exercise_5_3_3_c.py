...
tau    -   the delay between the two pulses
...
def EpulseDouble(t) :
    return Epulse(t) + Epulse(t - Tpulse - tau)
...
Tvector = np.arange(0, 2*Tpulse + tau + Tafter, dt)
# First plot: Laser pulse (with progagation progress)
ax1.plot(Tvector, EpulseDouble(Tvector), color = 'black')
ax1.set_xlabel('Time [a.u.]')
ax1.set_ylabel('E(t) [a.u.]')
line1, = ax1.plot([t], [EpulseDouble(t)], '*', color = 'red')
...
while t < 2*Tpulse + tau:
  ...
  DynamicalProp = np.diag(np.exp(-1j*x*EpulseDouble(t+dt/2)*dt))
  ...
  line1.set_xdata([t])
  line1.set_ydata([EpulseDouble(t)])
