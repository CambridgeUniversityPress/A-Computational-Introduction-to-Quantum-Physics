...
%
% The purely time-dependent laser pulse is modelled as the sum of 
% two sin^2-type pulses with a time-delay between them.
%
...
Epulse = @(t) (t>0 & t<Tpulse)*E0.*sin(pi*t/Tpulse).^2.*...
    sin(omega*t);
DoublePulse = @(t) Epulse(t) + Epulse(t - Tpulse - tau);
...
tVector = 0:dt:(2*Tpulse + tau + Tafter);
PulseVector = DoublePulse(tVector);
figure(1)
...
axis([0 2*Tpulse + tau + Tafter -1.1*E0 1.1*E0])
hold on
plPulse = plot(0, 0, 'r*', 'linewidth', 2);
hold off
xlabel('Time')
set(gca, 'fontsize', 15)
% Wave packet
...
% First: For  t< Tpulse
t = 0;
while t < 2*Tpulse + tau
  % First half step, H_0
  Psi = UhalfH0*Psi;
  % Interaction
  Psi = diag(exp(-1i*DoublePulse(t+dt/2)*x*dt))*Psi;
  % Second half step, H0
  Psi = UhalfH0*Psi;
    
  % Plot propagation of wave function and pulse on the fly
  set(plPulse, 'xdata', t);  
  set(plPulse, 'ydata', DoublePulse(t));  
...
Pion = trapz(x, abs(PsiUnbound).^2);
disp(['Ionization probability: ', num2str(100*Pion), '%'])
% Fourier transform
